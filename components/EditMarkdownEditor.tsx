...
interface EditMarkdownEditorProps {
    user: User
    post: {
        id: string
        title: string
...
export default function EditMarkdownEditor({ user, post }: EditMarkdownEditorProps) {
    const router = useRouter()
    const [title, setTitle] = useState(post.title)
    const [content, setContent] = useState(post.content || '')
    const [status, setStatus] = useState<'draft' | 'published' | 'private'>(post.status)
    ...
    const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (error) {
            ...
            insertText(`![${file.name}](${urlData.publicUrl})`)
        }
        ...
        <div className="flex h-screen flex-col overflow-hidden bg-[var(--bg-primary)] text-[var(--text-body)]">
            ...
            <header className="flex h-14 shrink-0 items-center justify-between border-b border-[var(--border)] px-4">
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        <span className="text-sm font-semibold text-[var(--text-primary)]">글 수정</span>
                ...
                <div className="flex items-center gap-3">
                    ...
                    >
                        {isSaving ? <div className="h-4 w-4 animate-spin rounded-full border-2 border-[var(--text-muted)] border-t-transparent" /> : (
                    ...
                    <div className="relative">
                        ...
                        {showStatusMenu && (
                            <>
                                ...
                                <div className="absolute right-0 top-full mt-2 w-44 rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-1 shadow-xl z-20">
                                    <button
                                    ...
                                    <button
                                        ...
                                        className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm text-[var(--text-body)] hover:bg-[var(--bg-card-hover)] hover:text-[var(--text-primary)]"
                                    >
                                        <svg className="h-4 w-4 text-[var(--text-faint)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" />
                                        </svg>
                                        임시저장으로 전환
                                    </button>
            ...
            <div className="flex h-10 shrink-0 items-center justify-between border-b border-[var(--border)] bg-[var(--bg-muted)] px-4">
                <div className="flex items-center gap-1">
                    <button onClick={() => insertText('**', '**')} className="flex h-8 w-8 items-center justify-center rounded text-[var(--text-muted)] hover:bg-[var(--bg-input)] hover:text-[var(--text-primary)]" title="Bold"><span className="font-bold">B</span></button>
                    <button onClick={() => insertText('*', '*')} className="flex h-8 w-8 items-center justify-center rounded text-[var(--text-muted)] hover:bg-[var(--bg-input)] hover:text-[var(--text-primary)]" title="Italic"><span className="italic">I</span></button>
                    <button onClick={() => insertText('~~', '~~')} className="flex h-8 w-8 items-center justify-center rounded text-[var(--text-muted)] hover:bg-[var(--bg-input)] hover:text-[var(--text-primary)]" title="Strikethrough"><span className="line-through">S</span></button>
                    <div className="mx-2 h-4 w-px bg-[var(--border)]" />
                    <button onClick={() => insertText('## ')} className="flex h-8 w-8 items-center justify-center rounded text-[var(--text-muted)] hover:bg-[var(--bg-input)] hover:text-[var(--text-primary)] font-bold" title="Heading 2">H2</button>
                    <button onClick={() => insertText('### ')} className="flex h-8 w-8 items-center justify-center rounded text-[var(--text-muted)] hover:bg-[var(--bg-input)] hover:text-[var(--text-primary)] font-bold" title="Heading 3">H3</button>
                    <div className="mx-2 h-4 w-px bg-[var(--border)]" />
                    <button onClick={() => insertText('> ')} className="flex h-8 w-8 items-center justify-center rounded text-[var(--text-muted)] hover:bg-[var(--bg-input)] hover:text-[var(--text-primary)] text-lg font-serif font-bold" title="Quote">"</button>
                    ...
                    <input ref={imageInputRef} type="file" accept="image/jpeg,image/png,image/gif,image/webp,image/svg+xml" className="hidden" onChange={handleImageUpload} />
                </div>
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                <div className="w-1/2 overflow-y-auto p-8 lg:px-12">
                    ...
                    <h1 className="mb-8 text-4xl font-bold text-[var(--text-primary)] leading-tight">{title || '제목을 입력하세요'}</h1>
            ...
            <footer className="flex h-8 shrink-0 items-center justify-between border-t border-[var(--border)] bg-[var(--bg-primary)] px-4 text-xs text-[var(--text-faint)]">
                ...
                <div className="flex items-center gap-2">
                    <span className={`h-2 w-2 rounded-full ${status === 'published' ? 'bg-emerald-500' : status === 'private' ? 'bg-amber-500' : 'bg-[var(--text-faint)]'}`} />
                    {statusLabel[status]}
                </div>
            </footer>
        </div>
